...
namespace Roguelike.Optimization
{
    ...
    public class HierarchicalSimulationRunner : ISimulationRunner
    {
        ...
        public SimulationStats Run(HierarchicalGenome genome, int seed)
        {
            var simHero = DeepClone(_baseHero);
            var simCards = DeepClone(_baseCards);
            ...
            while (runState.CurrentState != GameState.GameOver)
            {
                ...
                switch (runState.CurrentState)
                {
                    ...
                    case GameState.InShop:
                        RunShop(controller, runState, stats);
                        break;

                    case GameState.AwaitingReward:
                        int goldBefore = runState.TheHero.CurrentGold;
                        var cardRewardIndex = _agent.ChooseCardReward(runState);
                        controller.ConfirmRewards(cardRewardIndex);
                        stats.GoldCollected += runState.TheHero.CurrentGold - goldBefore;
                        break;
                }
            }
