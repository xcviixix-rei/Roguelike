...
namespace Roguelike.Optimization
{
    ...
    public class GeneticAlgorithm
    {
        ...
        public GeneticAlgorithm(BalanceSimulationRunner runner, FitnessEvaluator evaluator, CardPool baseCards, EnemyPool baseEnemies, EffectPool baseEffects)
        {
            _runner = runner;
        ...
        private void LogGenerationResults(int gen, dynamic best, StreamWriter log, TimeSpan elapsed, List<dynamic> rankedPopulation)
        {
            ...
            var genome = best.Genome as BalanceGenome;
            ...
            log.WriteLine("ROOM DISTRIBUTION:");
            log.WriteLine($"  Weights (Total: {b.TotalWeight:F0}):");
            log.WriteLine($"    Monster: {monsterWeight,5:F1} ({monsterWeight / b.TotalWeight,5:P0})");
            log.WriteLine($"    Elite:   {eliteWeight,5:F1} ({eliteWeight / b.TotalWeight,5:P0})");
            log.WriteLine($"    Event:   {eventWeight,5:F1} ({eventWeight / b.TotalWeight,5:P0})");
            log.WriteLine($"    Shop:    {shopWeight,5:F1} ({shopWeight / b.TotalWeight,5:P0})");
            log.WriteLine($"    Rest:    {restWeight,5:F1} ({restWeight / b.TotalWeight,5:P0})");
            log.WriteLine();
            log.WriteLine($"  Star Ratings:");
            log.WriteLine($"    Monster: {b.MonsterStarRatio:P0} are 2-star (Avg: {1.0f + b.MonsterStarRatio:F2}★)");
            log.WriteLine($"    Elite:   {b.EliteStarRatio:P0} are 4-star (Avg: {3.0f + b.EliteStarRatio:F2}★)");
            log.WriteLine();
            log.WriteLine($"  Derived Metrics:");
            log.WriteLine($"    Combat Rooms:   {b.CombatRoomPercentage:P0} (Monster + Elite)");
            log.WriteLine($"    Recovery Rooms: {b.RecoveryRoomPercentage:P0} (Rest + Shop)");
            log.WriteLine($"    Event Rooms:    {b.EventRoomPercentage:P0}");
            log.WriteLine($"    Avg Combat Difficulty: {b.AvgCombatDifficulty:F2}★");
            log.WriteLine($"    Rest Healing: {genome.RestHealingScalar:P0} of base (30%)");
            log.WriteLine();
            ...
            foreach (var card in ((IEnumerable<dynamic>)report.CardViability).Take(5))
            {
                string flag = "";
        ...
        private void PrintConsoleSummary(int gen, dynamic best)
        {
            ...
            Console.WriteLine($"  Recovery: {b.RecoveryRoomPercentage:P0}  (Healing: {genome.RestHealingScalar:P0})");
            Console.WriteLine($"  Event:    {b.EventRoomPercentage:P0}");
            Console.WriteLine();
        ...
        private BalanceGenome Crossover(BalanceGenome parent1, BalanceGenome parent2)
        {
            ...
            lock(_rngLock)
            {
                ...
                child.EliteStarRatio = (_rng.NextDouble() < 0.5) ? parent1.EliteStarRatio : parent2.EliteStarRatio;

                child.RestHealingScalar = (_rng.NextDouble() < 0.5) ? parent1.RestHealingScalar : parent2.RestHealingScalar;
        ...
        private void Mutate(BalanceGenome genome)
        {
            lock (_rngLock)
            {
                ...
                foreach (var roomType in genome.RoomTypeWeights.Keys.ToList())
                {
                    if (_rng.NextDouble() < MutationRate)
                    {
                ...
                foreach (var key in genome.CardActionScalars.Keys.ToList())
                {
                    ...
                    {
                        if (_rng.NextDouble() < MutationRate)
                            genome.CardActionScalars[key][i] += (float)(_rng.NextDouble() * 0.3 - 0.15);
                        genome.CardActionScalars[key][i] = Math.Clamp(genome.CardActionScalars[key][i], 0.5f, 1.5f);
                    }
                }
                ...
                foreach (var key in genome.EnemyActionValueScalars.Keys.ToList())
                {
                    ...
                    {
                        if (_rng.NextDouble() < MutationRate)
                            genome.EnemyActionValueScalars[key][i] += (float)(_rng.NextDouble() * 0.4 - 0.2);
                ...
                {
                    if (_rng.NextDouble() < MutationRate)
                        genome.EffectValueScalars[key] += (float)(_rng.NextDouble() * 0.3 - 0.15);
                    genome.EffectValueScalars[key] = Math.Clamp(genome.EffectValueScalars[key], 0.5f, 1.5f);
                }
            }
        }
        ...
        private void SaveBestGenomeReport(BalanceGenome genome, List<SimulationStats> evaluationResults, FitnessBreakdown breakdown, int generation)
        {
            ...
            var output = new
            {
                ...
                RoomDistribution = new
                {
                    ...
                    Percentages = new
                    {
                        Monster = monsterWeight / breakdown.TotalWeight,
                        Elite = eliteWeight / breakdown.TotalWeight,
                        Event = eventWeight / breakdown.TotalWeight,
                        Shop = shopWeight / breakdown.TotalWeight,
                        Rest = restWeight / breakdown.TotalWeight,
                        Combat = breakdown.CombatRoomPercentage,
                        Recovery = breakdown.RecoveryRoomPercentage
                    },
                    StarRatings = new
                    {
                        MonsterRatio = breakdown.MonsterStarRatio,
                        EliteRatio = breakdown.EliteStarRatio,
                        MonsterAvgStar = monsterAvgStar,
                        EliteAvgStar = eliteAvgStar,
                        OverallCombatDifficulty = breakdown.AvgCombatDifficulty
                    },
                    RestHealingScalar = genome.RestHealingScalar
                },
