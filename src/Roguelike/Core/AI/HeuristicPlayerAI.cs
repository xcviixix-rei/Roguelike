    ...
    public class HeuristicPlayerAI : IPlayerAgent
    {
        ...
        private int PredictDamage(int baseDamage, Hero hero, Enemy target)
        {
            ...
            if (strength != null)
            {
                ...
                finalDamage += strengthData.Intensity;
            }
            ...
            if (weakened != null)
            {
                var weakData = (StatusEffectData)weakened.SourceData;
        ...
        private CombatDecision? FindBestDefensiveMove(GameRun run, Hero hero, List<Enemy> enemies, int neededBlock)
        {
            ...
            for (int i = 0; i < hero.Deck.Hand.Count; i++)
            {
                ...
                if (blockAction != null)
                {
                    ...
                    if (frail != null)
                    {
                        var frailData = (StatusEffectData)frail.SourceData;
                ...
                if (weakAction != null && bestWeakTarget != null)
                {
                    ...
                    if (effectData != null)
                    {
                        bestWeakTarget.ActiveEffects.Add(new ActiveEffect(new StatusEffectData { EffectType = StatusEffectType.Weakened, Intensity = effectData.Intensity, IntensityType = IntensityType.Percentage, Duration = 1, Decay = DecayType.AfterXTURNS }));
