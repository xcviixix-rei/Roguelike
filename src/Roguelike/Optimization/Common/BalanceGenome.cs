...
namespace Roguelike.Optimization
{
    public class BalanceGenome
    {
        ...
        public void Randomize(CardPool cards, EnemyPool enemies, EffectPool effects, Random rng)
        {
            ...
            HeroHealthScalar = (float)(0.6 + rng.NextDouble() * 0.8);
            HeroStartGoldScalar = (float)(0.8 + rng.NextDouble() * 0.4);


            RoomTypeWeights[RoomType.Monster] = (float)(30 + rng.NextDouble() * 30);
            RoomTypeWeights[RoomType.Elite] = (float)(10 + rng.NextDouble() * 15);
            RoomTypeWeights[RoomType.Event] = (float)(10 + rng.NextDouble() * 25);
            RoomTypeWeights[RoomType.Shop] = (float)(5 + rng.NextDouble() * 10);
            RoomTypeWeights[RoomType.Rest] = (float)(5 + rng.NextDouble() * 10);


            MonsterStarRatio = (float)(0.3 + rng.NextDouble() * 0.4);
            EliteStarRatio = (float)(0.3 + rng.NextDouble() * 0.4);

            RestHealingScalar = (float)(0.5 + rng.NextDouble() * 1.0);
            ...
            foreach (var card in cards.CardsById.Values)
            {
                ...
                {
                    scalars.Add((float)(0.5 + rng.NextDouble() * 1.0));
                }
            ...
            {
                EnemyHealthScalars[enemy.Id] = (float)(0.4 + rng.NextDouble() * 1.4);
                ...
                foreach (var choice in enemy.ActionSet)
                {
                    ...
                    values.Add((float)(0.5 + rng.NextDouble() * 1.3));
                }
            ...
            {
                EffectValueScalars[effect.Id] = (float)(0.5 + rng.NextDouble() * 1.0);
            }
        }
    }
}
