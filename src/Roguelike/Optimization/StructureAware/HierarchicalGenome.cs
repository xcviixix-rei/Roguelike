...
namespace Roguelike.Optimization
{
    ...
    public class HierarchicalGenome
    {
        ...
        // FLOOR-SPECIFIC DIFFICULTY PROGRESSION (1 param)
        // Controls how difficulty scales across floors (0.0 = flat, 0.05 = 5% increase per floor)
        public float DifficultyProgressionRate { get; set; } = 0.03f;
        ...
        public void Randomize(Random rng)
        {
            ...
            // Floor difficulty progression: 0% to 6% per floor
            DifficultyProgressionRate = (float)(rng.NextDouble() * 0.06);
        ...
        public HierarchicalGenome Clone()
        {
            var clone = new HierarchicalGenome
            {
                ...
                DifficultyProgressionRate = this.DifficultyProgressionRate,
        ...
        /// <summary>
        /// Gets floor-specific difficulty multiplier for gradual progression
        /// Uses S-curve (sigmoid) for natural roguelike difficulty scaling
        /// </summary>
        public float GetFloorDifficultyMultiplier(int floor)
        {
            // Normalize floor to 0-1 range (floors 1-15)
            float t = (floor - 1) / 14.0f;
            
            // S-curve (sigmoid) for smoother progression
            // Starts slow, accelerates in middle, plateaus at end
            float sCurve = 1.0f / (1.0f + (float)Math.Exp(-10 * (t - 0.5f)));
            
            // Scale by progression rate
            // Example: 0.03 rate → ~0% at floor 1, ~15% at floor 15
            return 1.0f + DifficultyProgressionRate * 5.0f * sCurve;
        }
